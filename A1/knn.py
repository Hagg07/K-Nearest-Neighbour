...
def compute_distances_two_loops(x_train, x_test):
  ...
  A, B, C = x_train.shape[1:]
  x_train = x_train.reshape(num_train, A*B*C)
  x_test = x_test.reshape(num_test, A*B*C)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  p1 = torch.sum(x_train*x_train, dim = 1).reshape(-1, 1)
  p2 = torch.sum(x_test*x_test, dim =1).reshape(1, -1)
  dists = p1+p2
...
def predict_labels(dists, y_train, k=1):
  ...
  def get_example_pred(tops):
    ...
    major_repition = sorted_counts[0][1]
    top_repeats = [count for count in sorted_counts if count[1]==major_repition]
    vote = min([repeat for repeat in top_repeats])[0]
    return vote

  for i in range(len(dists[0])):
    distances_col = dists[:, i]
    values, indicies = torch.topk(input= distances_col, k=k, largest=False)
    top_preds = y_train[indicies].tolist()
    list_y.append(get_example_pred(top_preds))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
